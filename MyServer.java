...
try{
	...
	al.add(s);
	users.add(username);
	tellEveryOne("****** "+ username+" in "+(new Date())+" login my chat room");
	sendNewUserList();
    }
...
//	System.out.println("received from "+s.getPort());
	...
	tellEveryOne("****** "+username+" in "+(new Date())+" quit my chat room");
	sendNewUserList();
	al.remove(s);
	s.close();
...
center1.add(new JLabel("all of messages",JLabel.CENTER),"North");
...
east.add(new JLabel("all of members now",JLabel.CENTER),"East");
...
chatWindow.pack();
chatWindow.setTitle("login chat room");
...
if(s!=null)
{
JOptionPane.showMessageDialog(chatWindow,"are you sure to quit? ","Exit",JOptionPane.INFORMATION_MESSAGE);
logoutSession();
}
System.exit(0);
}
...
{
if(s==null)
 	{JOptionPane.showMessageDialog(chatWindow,"plz login first!!"); return;}
try{
...
if(temp==loginButton)
{
String uname=JOptionPane.showInputDialog(chatWindow,"input your nickname ");
if(uname!=null)
	clientChat(uname); 
}
...
if(s!=null)
{
JOptionPane.showMessageDialog(chatWindow,"are you sure quit now? ","Exit",JOptionPane.INFORMATION_MESSAGE);
logoutSession();
}
System.exit(0);
}
...
logoutButton.setEnabled(false);
loginButton.setEnabled(true);
chatWindow.setTitle("before chat, you need login!!!");
}
//////////////////////////
